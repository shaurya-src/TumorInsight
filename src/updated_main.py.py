...
def FCM():
    ...
    class FCM():
        ...
        def calculate_h(self):
            ...
            u_rolled = np.zeros((int(self.numPixels ** 0.5), int(self.numPixels ** 0.5)))
            kernel = np.ones((5,5))
            # kernel[2][2] = 4
            ...
            for i in range(self.n_clusters):
                ...
                u_rolled = self.U.transpose()[i].reshape(int(self.numPixels ** 0.5), int(self.numPixels ** 0.5))
        ...
        def segmentImage(self,image_count):
            ...
            for i in range(self.numPixels):
                self.Y[i] = self.C[self.result[i]].astype(np.int)
                #print(self.Y[i])
            self.Y = self.Y.reshape(int(self.numPixels ** 0.5), int(self.numPixels ** 0.5))
